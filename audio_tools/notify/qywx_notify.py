...
from audio_tools.binderapi import QywxConf

logger = logging.getLogger(__name__)
token_cache = Cache(maxsize=1000)
APP_USER_AGENT = f"htnanako/NDU:{os.environ.get('VERSION')}"
...
class QYWXNotify:
    ...
    def send_text_message(self, text_message):
        ...
        if access_token is None:
            logger.error("获取企业微信access_token失败，请检查你的corp_id和corp_secret配置")
            return
        data = json.dumps({
            'touser': self.to_user,
